...
function literal(scanner, allowBrackets) {
    ...
    while (readable(scanner)) {
        ...
        else if (isQuote$2(token) || isOperator(token) || isWhiteSpace$1(token) || isRepeater(token)) {
            break;
        }
...
function isUnquoted(code) {
    return !isNaN(code) && !isQuote$1(code) && !isSpace$1(code) && !isTerminator(code);
}
/**
...
function getUnquotedValue(value) {
    // Trim quotes
    if (isQuote$1(value.charCodeAt(0))) {
        value = value.slice(1);
    }
    if (isQuote$1(value.charCodeAt(value.length - 1))) {
...
function innerRange(source, start, end) {
    while (start < end && isSpace$1(source.charCodeAt(start))) {
        start++;
    }
    while (end > start && isSpace$1(source.charCodeAt(end - 1))) {
        end--;
    }
...
function extract(text, pos = text.length, options) {
    ...
    if (opt.lookAhead && cur(scanner) === 41 /* RightParenthesis */) {
        ...
        while (scanner.pos < len) {
            ch = cur(scanner);
            if (ch !== 41 /* RightParenthesis */ && !(opt.whitespace && isSpace$1(ch))) {
                break;
            }
            scanner.pos++;
        }
    ...
    while (scanner.pos >= 0) {
        ...
        else if (!((opt.whitespace && isSpace$1(ch)) || isSign(ch) || isOperator$3(ch))) {
            break;
        }
        scanner.pos--;
    }
    ...
    return {
        ...
        token(stream, state) {
            ...
            if (stream.eol() && state.braces.length && !state.parseError) {
                ...
                state.parseError = error$3(`No closing brace at ${pos}`, scanner);
...
function unexpectedCharacter(scanner, state) {
    state.parseError = error$3('Unexpected character at ' + scanner.pos, scanner);
        ...
        if (!rangeEmpty(innerRange)) {
            ...
            if (start.line !== end.line) {
                ...
                while (line <= end.line) {
                    ...
                    if (isSpace(substr(editor, indentRange))) {
